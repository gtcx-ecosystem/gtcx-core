...
The `@gtcx/schemas` package defines the Core12 compliance framework with 12 domain schemas, each containing multiple controls with nested validation rules. Maintaining separate TypeScript interfaces and JSON Schemas for this volume would be error-prone.
...
- Not language-agnostic: Rust crates and Python services can't consume Zod schemas directly
