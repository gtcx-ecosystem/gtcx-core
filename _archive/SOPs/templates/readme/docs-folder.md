...
This standard defines how documentation is organized in every repo. The structure is designed from first principles: 6 folders, each answering a distinct question. No project-specific folders at the root level. Every repo uses the same skeleton — only the content inside `specs/` differs.
...
**What does NOT belong here**: API endpoint docs for external users (→ `gitbook/api/`), per-service specs (→ `specs/`).
