# \docs/2-docs Structure Standard

> Canonical structure for `docs/` across all {organization-name} repositories.

This standard defines how documentation is organized inside `docs/` in every repo. The structure is designed from first principles: 6 folders, each answering a distinct question. No project-specific folders at the root level. Every repo uses the same skeleton — only the content inside `specs/` differs.

`docs/` is the documentation subfolder within the repo's `docs/` folder. For the full SOP structure (all numbered subfolders), see `sop-structure-standard.md`.
...
**What does NOT belong here**: API endpoint docs for external users (→ `docs/gitbook/api/`), per-service specs (→ `specs/`).
...
1. Copy this file into `docs/README.md`
