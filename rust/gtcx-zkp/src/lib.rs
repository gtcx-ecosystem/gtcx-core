...
//! Groth16 circuits (GCI threshold, asset ownership, location region) and a
//! Bulletproofs amount range proof are implemented, while additional circuits
//! (Bulletproofs, Groth16/Plonk) are tracked in the roadmap and will expand this
//! crate.
...
fn point_from_commitment(bytes: [u8; COMMITMENT_BYTES]) -> Result<RistrettoPoint> {
    let compressed = CompressedRistretto(bytes);
    compressed.decompress().ok_or(ZkpError::InvalidProofFormat {
        reason: "invalid amount commitment bytes".to_string(),
    })
}
...
pub fn bulletproofs_verify_amount_range(
    ...
    let amount_point = point_from_commitment(bundle.commitment)?;
