...
#[test]
fn test_kat_zw_diamond_origin_profile_proof_verifies() {
    use crate::types::{Groth16CircuitType, Groth16ProofBundle};

    let kat_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../artifacts/kat/groth16-zw-diamond-origin.kat.json");
    if !kat_path.exists() {
        return;
    }
    let kat_json: serde_json::Value =
    ...
    assert!(
        groth16_verify(&bundle).unwrap(),
        "zw-diamond-origin KAT must verify"
    );
}
