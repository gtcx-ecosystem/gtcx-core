...
Run this playbook when a CI gate fails on a PR or the main branch. Do not retry the same failing command repeatedly. Investigate, identify root cause, and fix at the source.

## Triage Order

Work through failures in this order — address each before moving to the next:
...
10. Rust gates (`cargo test`, `cargo clippy`) — parallel to TS gates
...
| Security gate                   | `security:threat-matrix` failure | Cryptographic Security Engineer                                                          |
...
| Performance budget exceeded     | Was a new operation added in the hot path? Was a benchmark run on an under-resourced machine? |
...
Fixes must address the root cause, not work around the gate. Acceptable fixes:

- Correct the code to match the spec
- Update the test to reflect a valid spec change (not to make it pass)
...
- The fix requires changing a security-sensitive package (`@gtcx/crypto`, `@gtcx/security`, `@gtcx/verification`, `@gtcx/identity`, Rust crates)
...
- If the fix reveals a spec gap: gap is added to `SOP/2-docs/4-operations/compliance/spec-to-code-traceability.md`
