...
All `@gtcx/*` packages use commodity-agnostic models. The domain model uses `commodityType: string` rather than commodity-specific types. Commodity-specific configuration (measurement units, quality thresholds, compliance domains, certification requirements) is registered at runtime via configuration, not compiled into core.
...
- Adding a new commodity requires zero code changes in `gtcx-core`
