    ...
    for (const metric of metrics) {
      ...
      if (metric.type === 'histogram') {
        ...
        for (const bucket of hist.buckets) {
          lines.push(`${metric.name}_bucket${labels.replace('}', `,le="${bucket.le}"}`) || `{le="${bucket.le}"}`} ${bucket.count}`);
        }
        ...
        for (const q of sum.quantiles) {
          lines.push(`${metric.name}${labels.replace('}', `,quantile="${q.quantile}"}`) || `{quantile="${q.quantile}"}`} ${q.value}`);
        }
...
export class ServiceMetrics {
  ...
  failure(operation: string, duration: number, errorCode: string, labels?: Record<string, string>): void {
    this.collector.increment(`${this.prefix}_${operation}_total`, { ...labels, status: 'failure' });
    this.collector.increment(`${this.prefix}_${operation}_errors_total`, { ...labels, error: errorCode });
    this.collector.histogram(`${this.prefix}_${operation}_duration_ms`, duration, labels);
  }
