 ...
 * documented in 01-docs/01-agents/governance/.
 *
 * Reads:
 *   - PR context from GITHUB_EVENT_PATH (workflow-injected)
 *   - System prompt from 01-docs/01-agents/governance/review-prompt.md
 *   - Schema from 01-docs/01-agents/governance/review-schema.json
...
const PROMPT_PATH = path.join(REPO_ROOT, '01-docs/01-agents/governance/review-prompt.md');
const SCHEMA_PATH = path.join(REPO_ROOT, '01-docs/01-agents/governance/review-schema.json');
const SELECT_SCRIPT = path.join(
  REPO_ROOT,
  '.github/03-platform/scripts/codeowner-review/select-playbooks.mjs'
);
...
Emit a single JSON document conforming to 01-docs/01-agents/governance/review-schema.json. Nothing else.`;
}
...
async function callProvider({ system, user }) {
  ...
  try {
    ...
    console.error(`Anthropic failed (${err.status}: ${err.message}); falling back to OpenAI`);
...
function parseAndValidate(rawText) {
  ...
  if (parsed.decision === 'APPROVE') {
    throw new Error(
      'SECURITY EVENT: model emitted decision=APPROVE — schema explicitly forbids this'
    );
  }
...
function formatReviewBody(review) {
  ...
  lines.push('---');
  lines.push(
    '_This review was generated by the AI CODEOWNER. The bot never approves — a human CODEOWNER must approve before merge. See [`01-docs/01-agents/governance/`](../../01-docs/01-agents/governance/)._'
  );
