...
const AUTO_DEV = join(ROOT, '01-docs/05-audit/auto-dev-state-tier5-commercial.md');
...
for (const item of lf.workSet.plan) {
  lines.push(
    `- **${item.storyId}** — ${item.title}${item.command ? ` (\`${item.command}\`)` : ''}`
  );
}
...
let roadmap = '';
const roadmapPath = join(ROOT, '01-docs/05-audit/execution-roadmap.md');
if (existsSync(roadmapPath)) {
  ...
  if (roadmap.includes('reconciliation_note:')) {
    const existing =
      roadmap.match(/reconciliation_note:\s*>-\s*\n([\s\S]*?)(?=\nsources:)/)?.[1] ?? '';
