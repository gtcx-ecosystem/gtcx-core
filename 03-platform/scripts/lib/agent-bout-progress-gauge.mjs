...
export const BOUT_PROGRESS_DOC = '01-docs/04-ops/agent-bout-progress-gauge.md';
...
export function overlayFromLatestJson(config, root) {
  const latestPath = join(root, '01-docs/05-audit/latest.json');
  ...
  for (const [key, dim] of Object.entries(dims)) {
    ...
    if (typeof v === 'number') {
      dims[key] = { ...dim, current: v, syncedFrom: '01-docs/05-audit/latest.json' };
    }
  }
...
export function buildProgressGauge(root, { applyLatestJson = true } = {}) {
  ...
  for (const [key, bump] of Object.entries(boutDelta)) {
    if (deltaDims[key]) {
      deltaDims[key] = {
        ...deltaDims[key],
        afterBout: Math.min(deltaDims[key].ceiling ?? 10, (deltaDims[key].current ?? 0) + bump),
      };
    }
  }
  ...
  const activeTasks = (config.tasks ?? []).filter((t) => !['done', 'cancelled'].includes(t.status));
