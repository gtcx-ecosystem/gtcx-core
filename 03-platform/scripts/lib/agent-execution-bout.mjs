...
export const BOUT_DOC = '01-docs/04-ops/agent-execution-bout.md';
...
function storyFromP22Next(next) {
  if (!next) return null;
  const storyId = next.milestone ?? next.handoff ?? next.storyId ?? next.dimension ?? null;
...
export function buildExecutionBout(ctx) {
  ...
  const currentStoryId =
    (p22HumanOnly && implementable.length > 0 ? implementable[0].storyId : null) ??
    head?.storyId ??
...
export function attachExecutionBout(nextWork, ctx) {
  const bout = buildExecutionBout({
    ...ctx,
    launchFocus: ctx.launchFocus ?? nextWork?.launchFocus,
  });
...
export function readRepoContext(repoRoot) {
  ...
  const workplanPath = join(repoRoot, '01-docs/04-ops/tier-5-workplan-2026-06.md');
  let session = '';
  let workplan = '';
