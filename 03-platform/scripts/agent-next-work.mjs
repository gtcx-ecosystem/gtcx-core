 ...
 * Reads 01-docs/05-audit/moat-dimension-roadmap-10-10.md; emits JSON to stdout.
 */
...
const ROADMAP_PATH = join(REPO_ROOT, '01-docs/05-audit/moat-dimension-roadmap-10-10.md');
const SESSION_PATH = join(REPO_ROOT, '.baseline/memory/session.md');
const TIER5_WORKPLAN_PATH = join(REPO_ROOT, '01-docs/04-ops/tier-5-workplan-2026-06.md');
...
function formatTier5Selection(item, tier, reason) {
  return {
    next: {
      ...
      sprint: item.sprint,
      title: item.title,
      workplan: '01-docs/04-ops/tier-5-workplan-2026-06.md',
      framework: 'DTF-001',
    },
  ...
  for (const item of OPS_DOCS_QUEUE) {
    ...
    return {
      next: {
        ...
        title: item.title,
        paths: item.paths,
        workplan: '01-docs/05-audit/README.md',
      },
      selection: {
        tier: 'ops-docs',
        reason:
...
function certifiedPackManifestReady() {
  const latest = join(REPO_ROOT, '01-docs/05-audit/evidence/certified-pack-manifest-latest.json');
  try {
...
function core004EngineeringDone() {
  return existsSync(join(REPO_ROOT, CORE_004_ENGINEERING_DONE));
}
...
function selectBacklogClearGuidance() {
  ...
  const commercialDoc =
    '01-docs/04-ops/coordination/from-gtcx-core-tier5-commercial-unblock-2026-06-06.md';
  const blockerDoc = '01-docs/04-ops/coordination/core-004-xr402-blocker-2026-06-04.md';
  ...
  return {
      ...
      ? core004Done && launchPlanDone
        ...
        : core004Done
          ...
          : {
              ...
              outbound: blockerDoc,
              because:
                'Launch implement queue — commercial ceiling is LOI only, not repo-wide blocked',
            }
      : {
          owner: 'gtcx-core',
          action:
            'Class R: pnpm certified-pack:build-manifest && pnpm certified-pack:verify-manifest (in-session)',
          outbound: '01-docs/04-ops/certified-pack-pipeline.md',
    ...
    humanOnly: [
      ...
      ...(core004Done
        ? [
            {
              storyId: 'CORE-004-CEREMONY',
              title: 'ZKP trusted-setup transcript publish (sovereign ceremony)',
              authorityClass: 'S',
              owner: 'Custodian / human',
            },
          ]
        : []),
    ],
    ...
    statusUpdate: {
      ...
      approvalNeeded: packDone
        ...
        : undefined,
      deferred: core004Done
        ? 'DTF-5.5.3 optional; sovereign ZKP ceremony publish'
        : 'DTF-5.5.3 optional; CORE-004 transcript publish (Class R, parallel)',
    },
    communicationPolicy: {
...
const storyId = result.next?.milestone ?? result.next?.handoff ?? result.next?.storyId ?? '';
