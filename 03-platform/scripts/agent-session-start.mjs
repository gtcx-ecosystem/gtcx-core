...
let nextWork = null;
try {
  const raw = run('node 03-platform/scripts/agent-next-work.mjs');
...
try {
  const raw = run('node 03-platform/scripts/check-cross-repo-dependencies.mjs');
...
const nextLabel =
  nextWork?.next?.milestone ?? nextWork?.next?.handoff ?? nextWork?.next?.dimension ?? 'unknown';
...
if (existsSync(SESSION_PATH)) {
  ...
  if (sessionBody.includes(marker)) {
    ...
    const afterMatch = sessionBody
      .slice(sessionBody.indexOf(marker))
      .match(/\n## (?!Session bootstrap)/);
    const after = afterMatch
      ? sessionBody.slice(sessionBody.indexOf(marker) + afterMatch.index)
      : '';
...
const proceedBrief = {
  ...
  inputs: [
    '01-docs/04-ops/agent-work-selection.md',
...
const output = {
  ...
  proceedBrief: {
    ...
    boutDoc: '01-docs/04-ops/agent-execution-bout.md',
  },
  crossRepoDeps,
  ...
  template: '01-docs/04-ops/agent-proceed-brief-template.md',
  attestation: '01-docs/04-ops/agent-attestation-template.md',
};
...
if (executionBout) {
  ...
  log(`**Normative:** ${executionBout.normativeDoc ?? '01-docs/04-ops/agent-execution-bout.md'}`);
