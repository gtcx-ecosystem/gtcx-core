...
let nextWork = {};
try {
  const raw = execSync('node 03-platform/scripts/agent-next-work.mjs', {
    cwd: ROOT,
    encoding: 'utf8',
...
console.log(
  `Class R remaining: ${bout.stories?.length ?? 0} (current: ${bout.currentStoryId ?? 'none'})`
);
