 ...
 * Writes 01-docs/05-audit/auto-dev-data.json and refreshes 01-docs/05-audit/auto-dev-state.md.
 */
...
const AUTO_DEV_JSON = join(ROOT, '01-docs/05-audit/auto-dev-data.json');
const AUTO_DEV_MD = join(ROOT, '01-docs/05-audit/auto-dev-state.md');
const LATEST_JSON = join(ROOT, '01-docs/05-audit/latest.json');
const ROADMAP = join(ROOT, '01-docs/05-audit/execution-roadmap.md');
...
function parseRoadmapMeta(md) {
  ...
  const activePhase = md.match(/\*\*Active phase:\*\*\s*\*\*([^*]+)\*\*/)?.[1]?.trim() ?? 'unknown';
  return { lastReconciled, activePhase };
}

function fetchP22() {
  const out = execSync('node 03-platform/scripts/agent-next-work.mjs --json', {
    cwd: ROOT,
    encoding: 'utf8',
  });
...
function extInf002WitnessStatus() {
  const inboundAck = join(
    ROOT,
    '01-docs/04-ops/coordination/from-gtcx-infrastructure-ext-inf-002-ack-2026-06-07.md'
  );
...
function buildCrossRepoWitness() {
  ...
  return [
    ...
    },
    {
      ...
      manifest: '01-docs/05-audit/evidence/vendor-pen-test-pack-manifest-latest.json',
      path:
        extInfStatus === 'outbound-acknowledged'
          ? '01-docs/04-ops/coordination/from-gtcx-infrastructure-ext-inf-002-ack-2026-06-07.md'
          : '01-docs/04-ops/coordination/to-gtcx-infrastructure-ext-inf-002-vendor-pack-2026-06-05.md',
    },
  ];
}
...
function buildAutoDevData(p22, latest, launchFocus, roadmapMeta, gates, sessionMetrics) {
  ...
  return {
    ...
    launchFocus: launchFocus
      ...
      : null,
    executionRoadmap: {
      path: '01-docs/05-audit/execution-roadmap.md',
    ...
    sessionMetrics,
    paths: {
      latestJson: '01-docs/05-audit/latest.json',
      autoDevState: '01-docs/05-audit/auto-dev-state.md',
      autoDevData: '01-docs/05-audit/auto-dev-data.json',
      autoDevTier5: '01-docs/05-audit/auto-dev-state-tier5-commercial.md',
      agentWorkSelection: '01-docs/04-ops/agent-work-selection.md',
...
function buildAutoDevMd(data, p22) {
  ...
  const witnessRows = (data.crossRepoWitness ?? [])
    .map((w) => {
      const detail = w.storyId === 'EXT-INF-002' && w.coreInputReady ? ' (core pack ready)' : '';
...
function patchLatestJson() {
  ...
  if (!latest) return;
  latest.autoDevData = '01-docs/05-audit/auto-dev-data.json';
  latest.autoDevState = '01-docs/05-audit/auto-dev-state.md';
  latest.signalAssessment = {
    path: '01-docs/05-audit/signal-assessment-2026-06-05.md',
    roadmap: '01-docs/05-audit/signal-roadmap-2026-06-05.md',
    date: '2026-06-05',
    overall: 'L1-high',
    target: 'L3-low',
    axis: 'SIGNAL-E',
