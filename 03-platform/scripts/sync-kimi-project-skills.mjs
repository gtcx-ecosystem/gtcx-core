...
const FRAMEWORK = `${DOCS}/03-platform/tools/audit/audit-framework`;
const LANE_SCORING = `${DOCS}/03-platform/tools/audit/lane-scoring`;
const AUDIT_MANIFEST = `${DOCS}/03-platform/tools/audit/agent-commands/manifest.json`;
const ROADMAP = `${DOCS}/03-platform/tools/roadmap/roadmap-framework`;
...
function buildAuditBody(cmd) {
  ...
  const output = (c?.reportOutput || c?.output || `01-docs/05-audit/${cmd}-<DATE>.md`).replace(
    '<DATE>',
    'YYYY-MM-DD'
  );
...
- Lane audits: \`${DOCS}/03-platform/tools/audit/lane-audits/README.md\`
- Domain audits: \`${DOCS}/03-platform/tools/audit/domain-audits/README.md\`
