#!/usr/bin/env node
/**
 * Read-only seven-hub path drift scan (executable files only).
...
const SKIP = new Set(['node_modules', '.git', 'dist', '.turbo', 'coverage', '00-archive', '.next', 'prisma']);
const FORBIDDEN = [
  /01-01-docs\//,
  /11-03-platform\//,
...
console.log('layout drift check passed (gtcx-core).');
