...
const trackerPath = path.join(rootDir, '01-docs/quality/10-10-remediation-tracker.md');
...
function deriveKpisFromHistory(history) {
  ...
  return {
    ...
    windowDays:
      Number.isFinite(numericWindowDays) && numericWindowDays > 0 ? numericWindowDays : null,
...
function coverageFromFile(filePath) {
  ...
  return {
    statements: {
      ...
      pct: asPercent(statementCovered, statementTotal),
    },
    branches: {
      covered: branchCovered,
      total: branchTotal,
      pct: asPercent(branchCovered, branchTotal),
    },
    functions: {
      covered: functionCovered,
      total: functionTotal,
      pct: asPercent(functionCovered, functionTotal),
    },
    lines: {
      covered: statementCovered,
      total: statementTotal,
...
for (const pkg of criticalPackages) {
  const filePath = path.join(rootDir, `03-platform/packages/${pkg}/coverage/coverage-final.json`);
...
const computedCiPassRate =
  performanceBudget.pass && coverageThresholdPass && evidence.missingEvidenceCount === 0 ? 100 : 0;
...
const kpis = {
  ...
  flakyTestRatePercent: parseEnvNumber(
    'KPI_FLAKY_TEST_RATE_PERCENT',
    historyDerivedKpis.flakyTestRatePercent
  ),
