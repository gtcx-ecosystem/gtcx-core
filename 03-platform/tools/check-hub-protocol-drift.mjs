...
import { allManifestSlugs, collectHubProtocolEntry, resolveHubRoot } from './lib/hub-protocols.mjs';
...
const manifest = JSON.parse(
  readRepoFile('01-docs/01-agents/agent-protocols-manifest.json') ?? '{}'
);
let snapshot = {};
try {
  snapshot = JSON.parse(
    readRepoFile('01-docs/01-agents/agent-protocols-hub-snapshot.json') ?? '{}'
  );
} catch {
  failures.push('Invalid 01-docs/01-agents/agent-protocols-hub-snapshot.json');
}
...
if (!hubRoot) {
  ...
  const readme = readRepoFile(join(hubRoot, '01-docs/governance/protocols/README.md'), true);
  if (readme) {
    for (const id of [
      '26-agent-proceed-confirmation',
      '27-agent-execution-obligation',
      '28-agent-authority-classification',
    ]) {
  ...
  if (update) {
    ...
    writeFileSync(
      join(ROOT, '01-docs/01-agents/agent-protocols-hub-snapshot.json'),
  ...
  if (snapshotEmpty && !update) {
    warnings.push(
      'Hub snapshot empty — run pnpm agent:hub-snapshot:update (once per hub change review cycle)'
    );
  }
}
