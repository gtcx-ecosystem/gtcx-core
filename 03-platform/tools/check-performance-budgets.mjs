...
const strictTrend =
  process.argv.includes('--strict-trend') || isTruthy(process.env.PERF_ENFORCE_TREND);
...
function getTrendConfig(budgets) {
  ...
  return {
    ...
    maxRegressionPercent:
      Number.isFinite(maxRegressionPercent) && maxRegressionPercent >= 0
        ? maxRegressionPercent
        : 10,
...
function main() {
  ...
  for (const key of metricKeys) {
    ...
    if (
      ...
      historicalSamples[historicalSamples.length - 1] === observed
    ) {
