...
try {
  const { traced, withTrace, createCategoryLogger } = await import(
  ...
  if (typeof withTrace !== 'function') {
    console.error('withTrace is not a function');
    process.exit(1);
  }
  ...
  const result = withTrace(() => 42, 'test');
  if (result !== 42) {
    console.error('withTrace no-op did not execute function');
    process.exit(1);
  }
