...
**Owner:** Cryptographic Security Engineer (`01-docs/01-agents/roles/crypto-security-engineer.md`)
...
| CC1.3   | Establishes structure, authority, and responsibility                      | ✓                | `01-docs/01-agents/roles/` (4 agent roles), `.github/CODEOWNERS` (dual-reviewer enforcement), `01-docs/01-agents/governance/` (AI CODEOWNER pattern) |
...
| Control | Requirement                                        | gtcx-core status | Evidence                                                                                                                                          |
| ------- | -------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| CC2.1   | Obtains or generates relevant, quality information | ✓                | Threat model (`01-docs/09-security/threat-model.md`), STRIDE analysis, attack tree, fuzz campaign results                                         |
| CC2.2   | Internally communicates control responsibilities   | ✓                | `01-docs/01-agents/governance/`, `security-incident-runbook.md`, agent role docs                                                                  |
| CC2.3   | Communicates with external parties                 | ✓                | `SECURITY.md` (vulnerability reporting), `01-docs/08-gtm/` (sandbox regulator pack), public advisories template in `security-incident-runbook.md` |
...
| Control | Requirement                     | gtcx-core status | Evidence                                                                                                                    |
| ------- | ------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------- |
| CC3.1   | Specifies suitable objectives   | ✓                | `README.md` engineering standards, `01-docs/architecture/overview.md`                                                       |
| CC3.2   | Identifies and analyzes risk    | ✓                | STRIDE table in threat model; attack tree in `01-docs/09-security/attack-tree-signing.md`                                   |
...
| Control | Requirement                                           | gtcx-core status | Evidence                                                                                                                  |
| ------- | ----------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------- |
| CC5.1   | Selects and develops control activities               | ✓                | 21 CI gates, `03-platform/tools/check-*.mjs` enforcement (architecture, secrets, threat matrix, crypto deps, ops prereqs) |
...
| CC5.3   | Deploys controls through policies and procedures      | ✓                | `CLAUDE.md` (hard rules), `01-docs/devops/runbooks/quality-runbook.md`, `security-incident-runbook.md`                    |
...
| CC6.6   | Implements logical access controls for boundaries                          | ✓                | Architecture boundary check (`03-platform/tools/check-package-boundaries.mjs`); forbidden-imports table; redactSecrets default sanitizer                      |
...
| CC9.2   | Assesses and manages risks associated with vendors and business partners              | ⚠                | `03-platform/tools/check-crypto-deps.mjs` (allowlist for `@noble/*`), `cargo-deny`, Dependabot — but no formal vendor management policy document                                    |
...
| PI1.2   | System processing is complete, valid, accurate, timely, authorized | ✓                | Property-based tests (25 properties in `03-platform/packages/crypto/tests/property-based.test.ts`), 9.9M fuzz runs / 0 crashes |
...
- [`01-docs/09-security/threat-model.md`](../security/threat-model.md) — STRIDE + threat actors + mitigations
- [`01-docs/09-security/attack-tree-signing.md`](../security/attack-tree-signing.md) — attack-tree analysis for signature forgery
...
- [`01-docs/10-compliance/pci-dss-scope.md`](./pci-dss-scope.md) — zero-CHD determination
- [`01-docs/10-compliance/sox-controls.md`](./sox-controls.md) — SOX ITGC mapping
