---
title: '1-agentic Integration — gtcx-core'
status: 'current'
date: '2026-05-27'
owner: 'gtcx-core'
...
1-agentic        — GTCX internal AI development platform (runs on Baseline)
      ↓
01-docs/01-agents/   — Per-repo expression: roles, safety rules, playbooks for gtcx-core
```
...
| ------------------------------- | --------------------------------------------- | ----------- |
| Role definitions                | `01-docs/01-agents/roles/`                    | Done        |
| Safety rules                    | `01-docs/01-agents/workflows/safety-rules.md` | Done        |
| Task playbooks                  | `01-docs/01-agents/workflows/tasks/`          | Done        |
| Coordination protocol           | `01-docs/01-agents/structure/coordination.md` | Done        |
...
The four roles in `01-docs/01-agents/roles/` reference archetypes at `1-agentic/archetypes/`. Those canonical definitions do not yet exist. `1-agentic` must produce:
...
When `1-agentic` defines the provisioning model, each role file in `01-docs/01-agents/roles/` will need a provisioning section: what context is loaded at session start, what tools are permitted, and what approval gates are enforced.

### 3. Coordination runtime

If `1-agentic` provides a runtime for multi-agent coordination (task routing, escalation, handoffs), the coordination model in `01-docs/01-agents/structure/coordination.md` will need to map to those runtime primitives.
