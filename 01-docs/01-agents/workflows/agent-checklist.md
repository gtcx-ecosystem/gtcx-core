...
- [ ] Check `01-docs/architecture/overview.md` for dependency rules before adding an import
...
Then add the tier-specific gates from `01-docs/01-agents/workflows/risk-tier-gates.md`.
Examples:
...
For `security-sensitive` and `release-sensitive` work, prepare evidence using `01-docs/01-agents/workflows/agent-evidence-template.md`.
...
When a red flag appears: run the Quick Recovery protocol in `01-docs/01-agents/onboarding/context-recovery.md`.
