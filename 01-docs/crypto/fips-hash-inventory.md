...
**Scope:** `03-platform/packages/crypto/`, `rust/gtcx-crypto/`, `rust/gtcx-zkp/`

---

## 2. Inventory by Component

### 2.1 TypeScript — `03-platform/packages/crypto/`

| Function            | File                              | Algorithm                | Backend                                 | FIPS Status  | Fallback when `GTCX_FIPS_MODE=1` |
| ------------------- | --------------------------------- | ------------------------ | --------------------------------------- | ------------ | -------------------------------- |
| `hash256`           | `03-platform/src/hashing.ts`      | SHA-256                  | `@noble/hashes/sha256` or native crypto | **Approved** | Native `node:crypto` SHA-256     |
| `hash512`           | `03-platform/src/hashing.ts`      | SHA-512                  | `@noble/hashes/sha512` or native crypto | **Approved** | Native `node:crypto` SHA-512     |
| `hash`              | `03-platform/src/hashing.ts`      | SHA-256 / SHA-512        | `@noble/hashes/*` or native crypto      | **Approved** | Same algorithm via native crypto |
| `hashObject`        | `03-platform/src/hashing.ts`      | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `doubleHash256`     | `03-platform/src/hashing.ts`      | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `createCommitment`  | `03-platform/src/hashing.ts`      | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `verifyCommitment`  | `03-platform/src/hashing.ts`      | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `combineHashes`     | `03-platform/src/hashing.ts`      | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `generateSalt`      | `03-platform/src/hashing.ts`      | N/A (CSPRNG)             | `crypto.getRandomValues`                | **Approved** | OS CSPRNG (SP 800-90A)           |
| `buildMerkleTree`   | `03-platform/src/proofs.ts`       | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
| `verifyMerkleProof` | `03-platform/src/proofs.ts`       | SHA-256                  | `hash256` wrapper                       | **Approved** | SHA-256                          |
...
| ------------------ | -------------------------------------- | ------------------ | --------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------ |
| `sha256`           | `03-platform/src/hashing/mod.rs`       | SHA-256            | `sha2` (RustCrypto)               | **Approved**      | Delegates to `sha2::Sha256`; when `--features fips` is enabled, AWS-LC path replaces this via provider abstraction |
| `sha512`           | `03-platform/src/hashing/mod.rs`       | SHA-512            | `sha2` (RustCrypto)               | **Approved**      | Delegates to `sha2::Sha512`; same FIPS path as SHA-256                                                             |
| `blake3`           | `03-platform/src/hashing/mod.rs`       | BLAKE3             | `blake3`                          | **Supplementary** | Not NIST-approved; disabled in FIPS mode                                                                           |
| `blake3_keyed`     | `03-platform/src/hashing/mod.rs`       | BLAKE3 (keyed)     | `blake3`                          | **Supplementary** | MAC construction; not FIPS-approved                                                                                |
| `blake3_derive`    | `03-platform/src/hashing/mod.rs`       | BLAKE3 (KDF)       | `blake3`                          | **Supplementary** | Key derivation; not FIPS-approved                                                                                  |
| `sign` (Ed25519)   | `03-platform/src/signing/ed25519.rs`   | SHA-512 (internal) | `ed25519-dalek` / `sha2`          | **Approved**      | Ed25519 uses SHA-512 internally; FIPS 186-5 approved algorithm                                                     |
| `sign` (P-256)     | `03-platform/src/signing/p256.rs`      | SHA-256            | `aws-lc-rs` (with `fips` feature) | **Approved**      | CMVP #4816 validated                                                                                               |
| `sign` (secp256k1) | `03-platform/src/signing/secp256k1.rs` | SHA-256            | `k256` / `sha2`                   | **Approved**      | ECDSA with SHA-256; FIPS 186-4 approved                                                                            |
...
| ---------------------------- | -------------------------------- | --------------------- | ---------------------------------- | ----------------- | --------------------------------------------------------- |
| `sha256_digest`              | `03-platform/src/utils.rs`       | SHA-256               | `ark-crypto-primitives` (`Sha256`) | **Approved**      | Used for commitment pre-computation outside the circuit   |
| `Sha256Gadget::evaluate`     | `03-platform/src/groth16/mod.rs` | SHA-256 (R1CS gadget) | `ark-crypto-primitives`            | **Approved**      | SHA-256 constraints inside Groth16 circuits               |
| `PathVar::verify_membership` | `03-platform/src/groth16/mod.rs` | SHA-256 (Merkle path) | `ark-crypto-primitives`            | **Approved**      | Merkle tree uses `Sha256Gadget` for leaf and inner hashes |
| `commit`                     | `03-platform/src/commitment.rs`  | BLAKE3                | `blake3`                           | **Supplementary** | Lightweight hash-commitment proofs (non-Groth16 fallback) |
| `generate_proof`             | `03-platform/src/commitment.rs`  | BLAKE3                | `blake3`                           | **Supplementary** | Hash-commitment baseline proofs                           |
...
### 4.1 TypeScript Path (`03-platform/packages/crypto/`)
...
4. No BLAKE3 fallback exists in `03-platform/packages/crypto/` itself.
...
- `rust/gtcx-zkp/03-platform/src/commitment.rs`
