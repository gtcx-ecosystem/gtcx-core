...
Per-package specifications for **24** TypeScript workspace packages under `03-platform/packages/` (matches `pnpm architecture:check`), **4** shared config workspace packages, and **6** Rust crates in `gtcx-core`.

**Published to npm:** 22 packages on the current provenance train (`pnpm provenance:check-npm:strict`). Workspace-only: `@gtcx/ai-eval` (release tooling), `@gtcx/eap` (EAP CLI — see `03-platform/packages/eap/README.md`).
...
| `@gtcx/eap`             | —                                                         | Enterprise Auth Pack CLI (workspace-only; `03-platform/packages/eap/README.md`)                         |
...
These live under `03-platform/packages/config/` and support the workspace/tooling layer rather than the main runtime API surface.

| Package                     | Location                                    | Description                       |
| --------------------------- | ------------------------------------------- | --------------------------------- |
| `@gtcx/eslint-config`       | `03-platform/packages/config/eslint/`       | Shared ESLint flat configuration  |
| `@gtcx/typescript-config`   | `03-platform/packages/config/typescript/`   | Shared TypeScript config presets  |
| `@gtcx/tsup-config`         | `03-platform/packages/config/tsup/`         | Shared `tsup` build presets       |
| `@gtcx/jurisdiction-config` | `03-platform/packages/config/jurisdiction/` | Shared jurisdiction configuration |
...
All packages follow the security framework in `01-docs/09-security/security-framework.md`. Security-sensitive packages (`@gtcx/crypto`, `@gtcx/crypto-native`, `@gtcx/security`, `@gtcx/verification`, `@gtcx/identity`, `rust/gtcx-crypto`, `rust/gtcx-zkp`) require Cryptographic Security Engineer review on every change.
