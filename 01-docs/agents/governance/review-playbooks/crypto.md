...
**Version:** 1.0.0
**Applies to:** `03-platform/packages/crypto/`, `03-platform/packages/crypto-native/`, `rust/gtcx-crypto/`, `rust/gtcx-zkp/`
...
**Severity:** `critical`
**References:** `01-docs/decisions/001-rust-for-cryptography.md`, `01-docs/decisions/005-ed25519-signing.md`
...
**Severity:** `high`
**References:** `01-docs/09-security/threat-model.md` (TC-002)
...
**Rule:** The `SigningProvider`, `HashProvider`, and `KeyStore` traits in `rust/gtcx-crypto/03-platform/src/provider/mod.rs` and `keystore.rs` are public API. Adding methods is a breaking change requiring a major version bump in the crate's Cargo.toml + an entry in the API surface baseline.
...
**Severity:** `high`
**References:** `01-docs/decisions/013-api-baseline-and-performance-budget-gates.md`
...
**Rule:** Every new public function in `rust/gtcx-crypto/03-platform/src/` that takes attacker-controlled bytes (signatures, keys, messages, hash inputs, ZKP proof bytes) requires a corresponding fuzz target in `rust/gtcx-crypto/fuzz/fuzz_targets/`.

**Violation pattern:**

- Diff adds a `pub fn` in `rust/gtcx-crypto/03-platform/src/` that accepts `&[u8]` and is not exclusively used in test-only contexts
...
- Diff does not modify `01-docs/09-security/threat-model.md`
...
**Rule:** Adding a new cryptographic algorithm (a new signing scheme, hash, KEM, or KDF) requires updating the algorithm-mapping table in `01-docs/09-security/fips-validation-boundary.md`.

**Violation pattern:**

- Diff adds support for a new algorithm name (regex match in code: `Ed448`, `Falcon`, `ML-DSA`, `Kyber`, `BLAKE2`, etc.)
- Diff does not modify `01-docs/09-security/fips-validation-boundary.md`
...
- Diff adds a new `pub fn sign` or `pub fn verify` in `rust/gtcx-crypto/03-platform/src/`
...
**Rule:** Adding a new dependency in `rust/gtcx-crypto/Cargo.toml` or `03-platform/packages/crypto/package.json` is permitted only for crates on the approved list (Check 1). Any other crypto-related dependency requires an ADR.
...
- No new ADR in `01-docs/decisions/` introducing the dependency
