...
- `01-docs/specs/core-spec.md` — confirm the new package is within scope
- `01-docs/architecture/overview.md` — confirm the dependency direction is valid
- `01-docs/decisions/011-package-dependency-boundaries.md` — boundary rules
...
If the new package introduces a new layer, a new dependency direction, or a new cross-package contract, write an ADR before any code. See `01-docs/01-agents/workflows/tasks/write-adr.md`.
...
```bash
mkdir -p 03-platform/packages/<name>/src
```
...
03-platform/packages/<name>/
  ...
  03-platform/src/index.ts          — public exports only
  03-platform/src/<name>.ts         — implementation
...
### 6. Update `03-platform/tools/check-package-boundaries.mjs`
...
Add an entry in `01-docs/10-compliance/spec-to-code-traceability.md` mapping the new package spec to its implementation module.
...
- [ ] `03-platform/tools/check-package-boundaries.mjs` updated (with prior human approval)
...
- [`01-docs/specs/core-spec.md`](../../../specs/core-spec.md) — scope and constraints
