...
If the crate introduces a new cryptographic algorithm, a new native binding contract, or a new FIPS boundary, write an ADR before any code. See `01-docs/01-agents/workflows/tasks/write-adr.md`.
...
rust/<crate-name>/
  ...
  03-platform/src/lib.rs     — public API
...
- Never use an unaudited cryptographic library — check `01-docs/09-security/` for approved libraries
