...
Add an explicit architecture boundary gate (`pnpm architecture:check`) backed by `03-platform/tools/check-package-boundaries.mjs`.  
The gate enforces:

- no forbidden layer imports for core packages (`types`, `crypto`, `domain`, `security`, `verification`)
- no cross-package internal source imports (`/03-platform/src/` deep imports)
