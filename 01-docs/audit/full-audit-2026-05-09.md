...
| Compliance Posture             | ✓      | GDPR/PCI/SOX docs in `01-docs/10-compliance/`; FIPS inheritance in `01-docs/09-security/fips-validation-boundary.md` |
...
| Trust       | Ready               | 14-doc evidence pack at `01-docs/08-gtm/`                 |
...
- `01-docs/decisions/014-runtime-substrate.md:5` is `Status: Accepted`. Per `CLAUDE.md:65`, ADR acceptance is a human decision. Likely correct in practice but worth audit-log mention.
...
- Decompose `security/03-platform/src/offline/storage.ts` (766 LOC) — kill the last LOC exception
- Replace `@gtcx/ai` `randomBytes(16).toString('hex')` with `randomUUID()` for traceIds

**Moat opportunities (90-day copy test):**

- **AI-native governance pattern** — `gtcx-agent` as CODEOWNER bot performing SAST/threat-model deltas. Would not be replicated in 90 days. Document publicly. _(Status: documented in `01-docs/01-agents/governance/` as of `7537089`.)_
- **Inheritable-FIPS-by-design** — `01-docs/09-security/fips-validation-boundary.md` reusable across the GTCX ecosystem; rare in OSS crypto libraries. Productize.
...
**Definition of Done:** Every doc claim about package count and ADR count matches `find packages -name package.json` and `ls 01-docs/decisions/0*.md`.
...
| #   | Task                                          | Files                                                | Effort  |
| --- | --------------------------------------------- | ---------------------------------------------------- | ------- |
| 1   | Send Zimbabwe pre-submission email            | `01-docs/08-gtm/09-pre-submission-email-zimbabwe.md` | 5m      |
| 2   | Send Namibia / Zambia / Ghana parallel emails | `01-docs/08-gtm/{10,11,13}-*`                        | 30m     |
| 3   | DRC engagement                                | `01-docs/08-gtm/12-engagement-brief-drc.md`          | 2h      |
| 4   | Iterate one-pager based on regulator feedback | `01-docs/08-gtm/01-security-posture.md`              | ongoing |
| 5   | Document first response as case study         | `01-docs/08-gtm/14-first-deal-case-study.md`         | 2h      |
...
| #   | Task                                                               | Files                                     | Effort |
| --- | ------------------------------------------------------------------ | ----------------------------------------- | ------ |
| 1   | ~~SOC2 Type 1 readiness gap analysis~~ — **DONE** (commit pending) | `01-docs/10-compliance/soc2-readiness.md` | DONE   |
| 2   | ~~Customer-facing trust portal~~ — **DONE** (commit pending)       | `01-docs/governance/trust-portal.md`      | DONE   |
| 3   | ~~SLSA L3 attestation in release~~ — **DONE** (commit pending)     | `01-docs/09-security/slsa-attestation.md` | DONE   |
| 4   | First reference-customer case study                                | `01-docs/08-gtm/case-studies/`            | 1d     |
