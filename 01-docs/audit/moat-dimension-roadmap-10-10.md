...
| Milestone                                | Score | Acceptance Criteria                                                                                                                   | Effort             | Gate                                                      |
| ---------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | --------------------------------------------------------- |
| M7.1 Threat model document               | 5→6   | Document: attacker capabilities (local vs remote), witness lifetime, what leakage would mean, acceptable risk posture                 | 0.5 day            | `01-docs/09-security/zkp-sidechannels.md`                 |
| M7.2 `uint64_is_ge` audit                | 6→7   | Static analysis or manual review confirming bit-decomposition path has no secret-dependent branches; or documented as acceptable risk | 1 day              | `01-docs/09-security/zkp-uint64-is-ge-audit.md` committed |
...
| M7.5 Third-party side-channel assessment | 9→10  | External lab confirms no exploitable timing leakage in constraint-generation path                                                     | 2–3 weeks external | Signed report in `01-docs/05-audit/`                      |
...
| M8.3 Gadget verification      | 5→7   | SMT proofs for SHA-256 gadget correctness (input → output matches reference) and Merkle path verification | 5 days    | CI script `03-platform/tools/verify-r1cs.sh` passes |
...
| Milestone                  | Score | Acceptance Criteria                                                                                                 | Effort    | Gate                                       |
| -------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------ |
| M9.1 Vendor selection      | 0→3   | SOW signed with NCC Group, Trail of Bits, or Least Authority; scope includes circuits, trusted setup, side channels | 1 week    | Contract in `01-docs/05-audit/contracts/`  |
| M9.2 Draft report + triage | 3→6   | Draft received; all Critical/High findings logged in tracker; remediation plan agreed                               | 2–3 weeks | Tracker in `01-docs/05-audit/remediation/` |
...
| M9.4 Final report          | 8→9   | Signed final report received; no open Critical/High findings                                                        | 1 week    | Report in `01-docs/05-audit/`              |
...
| Milestone                      | Score | Acceptance Criteria                                                                                                                                                               | Effort           | Gate                                                |
| ------------------------------ | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | --------------------------------------------------- |
| M10.1 FIPS boundary audit      | 7→8   | Complete inventory: every hash call in `03-platform/packages/crypto/` and `rust/gtcx-crypto/` tagged as FIPS-approved (aws-lc/sha2) or supplementary (blake3); no unlabeled paths | 1 day            | `01-docs/crypto/fips-hash-inventory.md` committed   |
...
| M10.3 Regulator attestation    | 9→10  | African regulator (or NIST CMVP liaison) signs letter accepting supplementary BLAKE3 usage with documented fallback to SHA-256; letter in `01-docs/10-compliance/`                | 2 weeks external | Letter scanned + committed                          |
...
| ------- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | ----------- |
| 0 (now) | 10  | 10  | 9.5 | 10  | 10  | 10  | 9   | 0   | 0   | 9.5 | **8.95**    |
...
| `smt-verify`           | Every PR (after M8.3) | `z3 03-platform/scripts/verify-r1cs.smt2`    |
