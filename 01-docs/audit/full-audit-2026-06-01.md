...
> **Five-lane model (since 2026-06-05):** Current readiness → [readiness-model.md](./readiness-model.md) · [latest.json](./latest.json). This 2026-06-01 report is engineering/legacy composite context.
...
- **[Medium] [Spec Fidelity] — README blocker stale after crypto-native 0.4.0 fix**  
  **Evidence:** `README.md:47,81` lists odd-length-hex as Sprint 2 blocker; `03-platform/packages/crypto-native/CHANGELOG.md:17-35` documents fix in 0.4.0; `03-platform/packages/crypto-native/tests/hex-validation.test.ts:143-271` property-tests rejection.  
...
- **[Low] [Spec Fidelity] — Rust P2P transport explicitly Phase 2**  
  **Evidence:** `rust/gtcx-network/03-platform/src/lib.rs:15-18` — types only; libp2p integration planned.  
...
- **[High] [Compliance] — External penetration test not engaged**  
  **Evidence:** `01-docs/09-security/pen-test-engagement-log.md:42-47` — vendor not selected, SoW unsigned.  
  **Impact:** Enterprise/sovereign procurement gate.  
  **Fix:** Execute Sprint 4 vendor selection per `01-docs/internal/vendor-outreach-pen-test.md`.
...
**Justification:** 18/21 `@gtcx/*` packages on npm (`01-docs/governance/trust-portal.md:72-96`); FIPS path documented; fuzz campaign zero crashes; sovereign engagement roadmap active. Blocked from full S3 by missing pen-test report, SOC 2 letter, and 3 unpublished substrate packages (`resilience`, `telemetry`, `runtime`).

### First realistic deal (next 90 days)

**Zimbabwe or Ghana minerals-commission sandbox** — regulator reviews trust portal + npm packages + sample verification flow. Staged email templates exist (`01-docs/08-gtm/09-pre-submission-email-zimbabwe.md`, `13-pre-submission-email-ghana.md`).
...
- `SR-002` human-in-loop enforced in `01-docs/01-agents/safety-rules.json:16-24`
...
| File Structure    | **10**    | Clean root; `03-platform/packages/`, `rust/`, `01-docs/` well partitioned                                           |
...
| **Incident Response** | **Mostly Ready**     | `01-docs/devops/runbooks/incident-runbook.md` — P0 crypto patch SLA 24h                      |
...
| 2   | Populate trust-portal provenance column            | Remediation | `01-docs/governance/trust-portal.md`                        | S      | Vendor risk teams need attestation links |
| 3   | Run `release:ga:evidence:check` post-publish       | Remediation | `quality/`, `artifacts/`                                    | S      | GA evidence chain complete               |
| 4   | Wire `pnpm ai:evaluate` in CI (non-blocking)       | Innovation  | `.github/workflows/ci.yml`, `03-platform/packages/ai-eval/` | M      | Automated spec-drift detection           |
...
| 2   | Send sandbox intro (ZW or GH)     | Remediation | `01-docs/08-gtm/09-*.md`, `13-*.md` | S      | Revenue trigger |
...
**Tracked in:** [01-docs/roadmap.md §4.10](../roadmap.md#410-gtcxai-eval--machine-readable-trust-scorecards-strategic-moat) · [engagement roadmap Phase 6](../agile/roadmap/engagement-readiness-trust-automation-phase-6.md) · [trust portal](../governance/trust-portal.md#machine-readable-trust-artifacts-roadmap)
