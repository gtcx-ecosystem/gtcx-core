...
**Finding:** 5 packages still use `export *` in main barrels (`domain`, `security`, `workproof`, `verification`, `crypto`). This defeats tree-shaking. **P1** — refactor to explicit named exports per `01-docs/01-agents/docs-standard-lightweight.md`.
...
| Sub-dimension        | Score | Evidence                                         |
| -------------------- | ----- | ------------------------------------------------ |
| Executive brief      | 9.0   | `01-docs/08-gtm/00-executive-brief.md` current   |
| Security posture     | 9.0   | `01-docs/08-gtm/01-security-posture.md` current  |
| Compliance matrix    | 8.5   | `01-docs/08-gtm/02-compliance-matrix.md` current |
...
**Finding:** Repo overview (`01-docs/overview/README.md`) created — new GTM asset for investor diligence. **P2** — send Zimbabwe email to start regulator engagement.

### 1.4 Repo / Folder Hygiene

| Sub-dimension        | Score | Evidence                                  |
| -------------------- | ----- | ----------------------------------------- |
| Doc structure        | 9.5   | Only `/01-docs/` and `_delete/` exist     |
...
**Finding:** `01-docs/01-agents/sessions/` created for cross-agent handoffs. `01-docs/01-agents/schemas/` created for machine-readable docs. **No issues.**
...
**Finding:** `03-platform/packages/config/tsup/base.mjs` updated — `splitting: true`, `releaseConfig` with `minify: true`. `rust/Cargo.toml` updated — `opt-level = "z"`, `strip = true`. **No regressions.**
...
Only `/01-docs/` and `_delete/` exist. No `_sop/`, `_cannon/`, or `wiki/` to consolidate.
...
| --------------------- | ----------------------------------------------------- | ------- |
| Machine-Readable Docs | `01-docs/01-agents/docs-standard-machine-readable.md` | Current |
| Lightweight App       | `01-docs/01-agents/docs-standard-lightweight.md`      | Current |

### 3.2 New Infrastructure

| Artifact                | Location                                      | Status                               |
| ----------------------- | --------------------------------------------- | ------------------------------------ |
| Frontmatter lint tool   | `03-platform/tools/check-doc-frontmatter.mjs` | Created, 229 docs failing (expected) |
| Bundle size budgets     | `benchmarks/bundle-size-budgets.json`         | Created, 21 packages                 |
| JSON schemas            | `01-docs/01-agents/schemas/`                  | 4 schemas created                    |
| Structured safety rules | `01-docs/01-agents/safety-rules.json`         | 8 rules                              |
| Machine-routing rules   | `01-docs/01-agents/routing-rules.json`        | 4 roles                              |
...
| Threat matrix missing | Created    | `01-docs/09-security/threat-control-matrix.md` |
| Repo overview missing | Created    | `01-docs/overview/README.md`                   |
...
| Threat matrix    | Created       | `01-docs/09-security/threat-control-matrix.md` exists | ✅ Confirmed |
| zkp.ts coverage  | 90.14%        | `03-platform/packages/crypto/coverage/lcov.info`      | ✅ Confirmed |
...
**Status:** Created — `01-docs/overview/README.md` (525 lines)
...
- Create `01-docs/09-security/threat-control-matrix.md` ✅
...
| Commit         | Description                                                                    |
| -------------- | ------------------------------------------------------------------------------ |
| `2c8e6e1`      | refactor(rust): split gtcx-zkp/03-platform/src/lib.rs into 8 modules           |
