...
3. **Engage pen-test vendor** — `01-docs/09-security/pen-test-scope.md`
...
- **[P1] `03-platform/packages/crypto/tests/signing.test.ts:329`** — `afterEach` imported from vitest missing, causing test failure. **Fix:** Add `afterEach` to vitest import. _(Closed in commit `f9ae145`.)_
- **[P2] `rust/gtcx-consensus/03-platform/src/lib.rs:839`** — 839 LOC; should be split into modules per `gtcx-node` refactor pattern.
- **[P2] `rust/gtcx-zkp/03-platform/src/groth16.rs:692`** — 692 LOC; should be split.
...
| S3  | P1       | Pen-test vendor not engaged                      | `01-docs/09-security/pen-test-scope.md` | Select vendor and execute engagement    |
...
**First realistic deal (next 90 days):** Zimbabwe RBZ sandbox pre-submission email (`01-docs/08-gtm/09-pre-submission-email-zimbabwe.md`).
...
| -------------- | ----- | ----------------------------------------------------------------- |
| Documentation  | 9.0   | `/01-docs/` canonical; 234 files; `_delete/` handled              |
...
Phase 2 skipped — repo has only `/01-docs/` documentation root. `_delete/` exists from prior cleanup but contains no competing roots. Routed directly to Phase 3.
...
| Adaptive low-bandwidth not implemented   | `03-platform/packages/connectivity` adaptive mode shipped | `09be49b` |
...
| Threat matrix 12/12      | Validator passes     | `03-platform/tools/check-threat-matrix.mjs` reads controls; T11/T12 flagged as gaps not failures | Same (9.0)                      |
...
| Adaptive low-bandwidth   | Not implemented (P2) | `03-platform/packages/connectivity` adaptive mode shipped; 26 tests                              | Upgrade Resilience +0.3         |
