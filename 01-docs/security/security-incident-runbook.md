...
3. **Notify downstream consumers** identified in `01-docs/architecture/`:
   - `gtcx-protocols`
   - `gtcx-platforms`
   - `gtcx-mobile`
   - `gtcx-intelligence`
...
2. **Map the impact surface.** Which packages, which versions, which downstream consumers are affected? Use the dependency graph in `01-docs/architecture/` and `quality/api-surface-baseline.json`.
3. **Identify the root cause.** Distinguish:
   ...
   - Supply chain compromise — fix requires content-pin allowlist update (`03-platform/tools/check-crypto-deps.mjs`) and may require yanking dependency
...
For cryptographic correctness issues, the soundness analysis discipline from `01-docs/01-agents/roles/crypto-security-engineer.md` applies. The 12,400-revocation-incident lesson encoded there is the reason this runbook exists in the first place.
...
5. **Run the full quality gate** sequence locally (`01-docs/devops/runbooks/quality-runbook.md`) before any merge. Skipping `--no-verify` is a hard-rule violation per `CLAUDE.md`.
...
4. **Publish the retrospective summary** in `01-docs/09-security/incidents/<id>-retrospective.md` (public — sandbox regulators value this signal).
...
The AI CODEOWNER (`01-docs/01-agents/governance/review-prompt.md`) defines a `bypass-ai-review` label for emergency hot-fixes when the AI provider is unreachable or producing pathological output.
