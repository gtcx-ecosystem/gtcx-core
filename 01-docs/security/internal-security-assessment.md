...
- **STRIDE analysis:** Complete, covering all 9 components. 18 threats identified with mitigations. See `01-docs/09-security/threat-model.md`.
- **Attack tree:** Signature forgery tree with 20 leaf nodes, each with mitigation or accepted risk. See `01-docs/09-security/attack-tree-signing.md`.
- **Threat control matrix:** 12 controls with evidence paths, validated in CI. See `01-docs/09-security/threat-control-matrix.md`.
...
| **Domain separation** | Hash prefixes prevent cross-context collision | Implemented in `@gtcx/crypto/03-platform/src/hashing.ts` |
...
| ESLint security    | `03-platform/packages/config/eslint/index.js` (Math.random ban) |
| Architecture check | `03-platform/tools/check-package-boundaries.mjs`                |
| Secret scan        | `03-platform/tools/check-secrets.mjs`                           |
| Threat matrix      | `03-platform/tools/check-threat-matrix.mjs`                     |
