...
| **Provenance is generated and signed by the build platform** | Platform-signed provenance, not user-controllable | `npm publish --provenance` invokes sigstore via GitHub OIDC; signatures are issued by GitHub's identity, not the publisher | `release.yml` → `03-platform/tools/publish-packages-provenance.mjs` (`pnpm pack` + `npm publish --provenance`) |
...
All seven Build Level 3 requirements are satisfied **by mechanism configuration**. The pipeline is documented above; no requirement relies on private infrastructure or non-public claims. **Registry attestations:** **21/21** packages at the 3.1.4 provenance baseline (2026-06-01, [release 26778909174](https://github.com/gtcx-ecosystem/gtcx-core/actions/runs/26778909174)); verify with `pnpm provenance:check-npm:strict`.
...
| Phase            | Action                                                                                                                                                                                                   | Owner                 |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| **Generation**   | `release.yml` publishes via `publish-packages-provenance.mjs` (`npm publish --provenance` per package)                                                                                                   | GitHub Actions runner |
...
- **Provenance manifest** — see [`03-platform/tools/generate-provenance-manifest.mjs`](../../03-platform/tools/generate-provenance-manifest.mjs). The internal manifest is written to `artifacts/provenance-manifest.json` and captures source SHA, build config, and dependency snapshot.
- **SBOM (CycloneDX)** — generated by Trivy on every CI build at `quality/release-evidence/`.
- **Crypto deps content-hash allowlist** — see [`03-platform/tools/check-crypto-deps.mjs`](../../03-platform/tools/check-crypto-deps.mjs). This independent supply-chain check complements SLSA's provenance coverage.
- **Reproducible build verifier** — see [`03-platform/tools/check-reproducible-build.mjs`](../../03-platform/tools/check-reproducible-build.mjs). This independent check complements SLSA's hermeticity ambitions and is separate from Build Level 3 requirements.
- **AI CODEOWNER review log** — `quality/ai-review-log/` is populated by [`gtcx-codeowner-action`](../../.github/03-platform/scripts/codeowner-review/). It provides independent change-management evidence even though no formal Source Level claim depends on it.
