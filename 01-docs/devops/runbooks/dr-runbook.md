...
5. **Communicate** — Post incident notice to `01-docs/09-security/security-incident-runbook.md` §5
...
npm view @gtcx/crypto@latest --json | jq '.dist.integrity' && \
  cat 03-platform/packages/crypto/dist/*.tgz.sha256
```
...
cargo test --features fips -- key_rotation && \
  pnpm test -- 03-platform/packages/crypto/tests/revocation.test.ts
```
...
- [ ] Incident post-mortem written in `01-docs/09-security/security-incident-runbook.md`
