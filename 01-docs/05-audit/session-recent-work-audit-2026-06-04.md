...
**Verification:** `node 03-platform/scripts/ecosystem/rollout-protocol-27.mjs --check` → **exit 0** (22 repos).
...
| `pnpm format:check`               | **1** — `.baseline/memory/session.md`, `01-docs/04-ops/agent-universal-instructions.md` (uncommitted) |
...
| ------- | --------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------ |
| SM      | `bash 03-platform/scripts/staging/populate-terminal-os-staging-sm.sh` | 0     | Secret `gtcx/terminal-os/staging/api-keys` exists                                                |
...
1. **W2-OPS-001 incomplete** — terminal-os pod not Ready; ECR image missing (`04-ship/kubernetes/overlays/staging/terminal-os/deployment.yaml:50`).
...
1. **Hygiene commands still “do not commit unless asked”** while master/lane audits mandate commit+push — agents may contradict themselves (`03-platform/tools/audit/audit-framework/claude-commands/doc-standard.md:32`).
