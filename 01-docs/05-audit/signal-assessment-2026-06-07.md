...
**Evidence anchors:** `01-docs/05-audit/latest.json` (engineering signoff **10/10**, bank-grade **8.9**), `pnpm agent:protocols:check` exit 0, CI `agent:next-work` + `agent:protocols:check`, Protocol 27 V-ladder in `AGENTS.md`, `backlogClear: true` witness wall.
...
- Protocol 22 selector (`03-platform/scripts/agent-next-work.mjs`) — `backlogClear`, `automatableExhausted`, Class S wall, commercial ceiling.
...
- Machine-readable audit: `01-docs/05-audit/latest.json`, `auto-dev-data.json`, five-lane readiness model.
...
- Work register (`01-docs/04-ops/agent-work-selection.md`) with Class R/S and DTF-5.5.4 commercial ceiling rules.
...
- `01-docs/01-agents/safety-rules.json` — SR-002 no raw AI approval; SR-001 never skip CI; structured enforcement.
...
- `01-docs/05-audit/latest.json` — lane indexes, GCR/IC tiers, verification gate registry.
...
- Four role files under `01-docs/01-agents/roles/` (protocol-architect, crypto-security-engineer, etc.).
...
- **Target:** Named lead + link [`ai-reliability-owner-2026-06-06.md`](https://github.com/gtcx-ecosystem/gtcx-protocols/blob/main/01-docs/04-ops/coordination/ai-reliability-owner-2026-06-06.md)
...
- **Implementation:** Add `01-docs/04-ops/agent-pr-checklist.md` (mirror protocols SIGNAL-003); link from `.github/pull_request_template.md`
...
- **Implementation:** `03-platform/scripts/test-agent-integration.mjs` — session-start JSON, next-work JSON, launch-focus schema, protocols check; wire in `ci.yml`
...
- [ ] **Week 7:** `01-docs/architecture/agent-topology-foundation-2026-Q3.md` — named engines + handoffs
