...
function prTouchesAgentPaths(event) {
  ...
  try {
    ...
    if (out.trim()) {
      return out.split('\n').some((f) =>
        AGENT_PATH_GLOBS.some((g) => f.startsWith(g) || f === g.replace(/\/$/, '')),
      );
    }
  } catch {
    // gh not available
...
function fail(msg) {
  console.error(msg);
  console.error('\nTemplate: docs/operations/agent-attestation-template.md');
  process.exit(1);
}
...
if (!text) {
  console.log('Usage: pnpm agent:attestation:check --pr | --staged [--file path] | --commit-msg path');
  process.exit(0);
}
