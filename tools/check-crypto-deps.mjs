 ...
 *
 * Run: `node tools/check-crypto-deps.mjs`
 */
    ...
    if (!Object.prototype.hasOwnProperty.call(allowedVersions, version)) {
      errors.push(
        `Disallowed version: ${pkgName}@${version} appears in pnpm-lock.yaml but is not in the allowlist.\n` +
          ...
          `  To approve, add the version+integrity hash to tools/check-crypto-deps.mjs and submit a PR.`
      );
      continue;
    }
  ...
  if (!found) {
    errors.push(
      `Tracked package ${pkgName} not found in pnpm-lock.yaml.\n` +
        `  Either the dependency was removed (update tools/check-crypto-deps.mjs) ` +
