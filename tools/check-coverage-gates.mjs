...
const CRITICAL_PACKAGES = [
  { name: '@gtcx/crypto', dir: 'packages/crypto', thresholds: { statements: 85, branches: 80, functions: 90, lines: 85 } },
  { name: '@gtcx/domain', dir: 'packages/domain', thresholds: { statements: 90, branches: 85, functions: 90, lines: 90 } },
  { name: '@gtcx/security', dir: 'packages/security', thresholds: { statements: 90, branches: 85, functions: 90, lines: 90 } },
  { name: '@gtcx/services', dir: 'packages/services', thresholds: { statements: 90, branches: 75, functions: 85, lines: 90 } },
  { name: '@gtcx/verification', dir: 'packages/verification', thresholds: { statements: 90, branches: 80, functions: 85, lines: 95 } },
];
...
for (const pkg of CRITICAL_PACKAGES) {
  ...
  for (const [metric, threshold] of Object.entries(pkg.thresholds)) {
    ...
    if (actual < threshold) {
      failures.push(
        `${pkg.name}: ${metric} ${actual.toFixed(2)}% < threshold ${threshold}%`
      );
    }
  }
}
...
console.log(
  `Coverage gate passed for ${CRITICAL_PACKAGES.length} critical packages.`
);
