...
const securityViolationLabels = process.env.KPI_SECURITY_VIOLATION_LABELS ?? 'security-policy-violation';
...
const docsApiDriftGates = new Set(['API surface baseline check', 'Validate markdown links', 'Generate API docs']);
...
async function main() {
  if (!token || !repository) {
    ...
    console.log(`KPI history collection skipped (missing GitHub auth): ${path.relative(rootDir, outPath)}`);
    return;
  }
  ...
  for (const workflowFile of workflows) {
    ...
    for (const run of runs) {
      ...
      runSummary.hasDocsApiDriftFailure = failureSignals.some((signal) => docsApiDriftGates.has(signal));
      runSummary.hasSecurityPolicyFailure = failureSignals.some((signal) => securityPolicyGates.has(signal));
