 ...
 *
 * Run: node tools/bench-connectivity-adaptive.mjs
 */
...
const { classifyProfile } = await import(
  join(rootDir, 'packages/connectivity/src/profiles.ts')
);

const { DEFAULT_PROFILE_CONFIG, adaptClientOptionsForProfile } = await import(
  join(rootDir, 'packages/connectivity/src/adapters/api-client.ts')
);

const { RequestBatcher } = await import(
  join(rootDir, 'packages/connectivity/src/batching.ts')
);
