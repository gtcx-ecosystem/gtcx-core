...
└─────────────────┘     └──────────────────┘     └─────────────────┘
    ...
    │  `docs/agents/evaluations/baseline-<date>.json`│
    └────────────────────────────────────────────────┘
```
...
| `AI_EVAL_OUTPUT_DIR`    | No       | Directory for scorecard artifacts (default: `docs/agents/evaluations/`) |
  ...
  "rules": [
    {
      ...
      "pattern": "packages/crypto/**|packages/crypto-native/**|rust/**",
    ...
    "findings": [
      {
        ...
        "file": "packages/crypto/src/signing.ts",
        "severity": "block",
...
1. Parse `docs/agents/sessions/` for session metadata (if available)
  ...
  evaluate:
    ...
    steps:
      ...
      - name: Run AI Evaluation
        ...
        env:
          AI_EVAL_BASELINE_PATH: docs/agents/evaluations/baseline-latest.json
...
Baselines are committed JSON scorecards stored in `docs/agents/evaluations/`.

```
docs/agents/evaluations/
...
```bash
pnpm ai:evaluate --output docs/agents/evaluations/baseline-$(date +%Y-%m-%d).json
ln -sf baseline-$(date +%Y-%m-%d).json docs/agents/evaluations/baseline-latest.json
```
...
| ---- | ------------------------------------------- | --------------------- | ------ | ---------- |
| 1    | `packages/ai-eval/` package scaffold        | Protocol Architect    | S      | 2026-05-20 |
...
- `gtcx-core/docs/roadmap.md` — AI Maturity Roadmap (Level 2→5)
- `gtcx-core/docs/agents/safety-rules.json` — Machine-readable safety rules
- `gtcx-core/docs/governance/model-cards/` — Published model cards
- `gtcx-core/docs/agents/sessions/` — Session metadata format
