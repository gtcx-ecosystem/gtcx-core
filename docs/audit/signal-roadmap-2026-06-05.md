...
source_assessment: docs/audit/signal-assessment-2026-06-05.md
...
SIGNAL-P in this repo ≈ L0 (no user-facing LLM). All phases advance **SIGNAL-E** only. Standard L1→L2 / L2→L3 checklists are adapted below; criterion numbers reference [SIGNAL.md transition tables](https://github.com/gtcx-ecosystem/gtcx-docs/blob/main/docs/governance/frameworks/SIGNAL.md).
...
- **Definition of done:**
  - `scripts/agent-next-work.mjs` resolves `suggest-persona` import
...
1. Change import to `gtcx-agentic/13-scripts/lib/suggest-persona.mjs` OR add shim at legacy path in gtcx-agentic
2. Extend `scripts/agent-cross-repo-deps-check.mjs` to assert import path exists
...
- **Definition of done:**
  - `docs/operations/agent-pr-checklist.md` exists with V-ladder table, attestation block, evidence paths
...
1. Copy structure from `docs/operations/agent-attestation-template.md`
2. Add section: agent-touch globs (`docs/agents/**`, `scripts/agent-*`, `.agent/**`, `.cursor/rules/**`)
...
- **Definition of done:**
  ...
  - `docs/operations/coordination/cross-repo-agent-bridge.md` Latest table updated
...
- **Definition of done:**
  - `docs/audit/auto-dev-data.json` includes `lastSessionGates: { command, exitCode }[]`
...
- **Definition of done:**
  - `docs/audit/latest.json` has `signalAssessment: { path, date, overall, target }`
...
- **Definition of done:**
  - Script or CI job validates links in `docs/coordination/outbound/*.md` and witness indexes
...
| L1→L2 #4 Gate telemetry             | `jq .lastSessionGates docs/audit/auto-dev-data.json` | Non-empty array                                                      |
...
jq '.signalAssessment.overall' docs/audit/latest.json
```
...
# Manual: docs/architecture/agent-topology-foundation-2026-Q3.md reviewed
```
