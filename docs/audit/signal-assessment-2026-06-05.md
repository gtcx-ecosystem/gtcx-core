...
supersedes: docs/audit/signal-assessment-2026-06-07.md
---
...
- **`pnpm agent:protocols:check` exit 1** — import `gtcx-agentic/scripts/lib/suggest-persona.mjs` broken (actual path `13-scripts/lib/`).
...
- `docs/agents/safety-rules.json` — SR-001/002 enforced.
...
- `docs/audit/latest.json` — five-lane readiness, GCR/IC tiers.
...
- Four role files under `docs/agents/roles/`.
...
- **Current:** `agent-next-work.mjs` imports missing `gtcx-agentic/scripts/lib/suggest-persona.mjs`
- **Target:** Import resolves to `13-scripts/lib/suggest-persona.mjs` or re-export shim; `pnpm agent:protocols:check` exit 0
- **Implementation:** Fix import in `scripts/agent-next-work.mjs` + `agent:cross-repo-deps:check` guard; mirror fix in gtcx-agentic if shim preferred
- **Dependencies:** None
...
- **Implementation:** `docs/operations/agent-pr-checklist.md`; link from PR template
...
- **Implementation:** `scripts/test-agent-integration.mjs` — session-start JSON, launch-focus schema, protocols check; wire in `ci.yml`
