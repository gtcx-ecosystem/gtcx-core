...
- **[High] [Structural Integrity] — Turbo build cycle workproof ↔ verification**  
  **Evidence:** `pnpm typecheck` reports cycle: `@gtcx/workproof#build → @gtcx/verification#build → @gtcx/workproof#build`. `packages/workproof/package.json:78` depends on `@gtcx/verification`; `packages/verification/package.json` devDepends `@gtcx/workproof` (integration tests).  
  **Impact:** Root `pnpm typecheck` / `pnpm build` can fail in clean CI depending on turbo graph resolution.  
  **Fix:** Move cross-package integration test to `packages/integration-tests` or `tests/integration/` with no published dep cycle; keep `CommodityOriginWitnessLike` in verification (no runtime workproof import).

- **[Medium] [Spec Fidelity] — README composite 9.5 vs Tier-5 in progress**  
  **Evidence:** `README.md:21` claims composite 9.5; `docs/operations/tier-5-workplan-2026-06.md:26` — Tier 5 technical **in progress** (DTF-5.2.3 next).  
...
- **[Medium] [Code Quality] — Verification LOC near policy ceiling**  
  **Evidence:** `packages/verification/src/certificates/templates.ts` ~468 LOC (per prior audit); new `commodity-origin-zk.ts` adds bridge layer.  
...
- **[Low] [Consistency] — `export *` barrels resolved in crypto**  
  **Evidence:** Prior master audit P1; `packages/crypto/src/index.ts` uses explicit exports (grep: no `export *`).  
...
- **[Low] [Operational Readiness] — Rust P2P transport Phase 2**  
  **Evidence:** `rust/gtcx-network/src/lib.rs` — types only; libp2p deferred.  
...
| Data Protection                | **Strong**                          | `rust/gtcx-crypto/src/fips.rs:32-36` — `GTCX_FIPS_STRICT=1` rejects non-FIPS algos; `sanitizeSecrets` in security/ai packages |
...
- **[High] [Compliance] — No third-party penetration test on deployed stack**  
  **Evidence:** `docs/gtm/gtm-reality-check-2026-06-02.md:60` — EXT-INF-002 open; internal fuzz 500K+ iterations (`docs/audit/fuzz-campaign-evidence-2026-05-21.md`).  
...
- **[Medium] [Data Protection] — Profile cert masks enforced pre-prove, not in-circuit**  
  **Evidence:** `rust/gtcx-zkp/src/circuit_profiles/validate.rs:64-83`; `docs/specs/packages/zkp-circuit-profiles.md` — by design.  
...
Stages assessed per `docs/gtm/gtm-reality-check-2026-06-02.md` (foundation library lens).
...
- AI-native traced APIs exist (`packages/verification/src/traced.ts`) but **commodity-origin ZK path is not yet traced** — only structural + optional crypto verify.
...
| File Structure    | **9**     | `packages/*`, `rust/*`, `docs/audit/` well partitioned                     |
...
- Agentic `agent:next-work` already in CI — expose milestone completion in machine-readable `docs/audit/auto-dev-state.json` for hub.
...
| --- | -------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------ |
| 1   | Extract verification↔workproof integration test to break turbo cycle | Remediation | `packages/verification/package.json`, new `tests/integration/commodity-origin-zk.test.ts` or `packages/integration-tests/` | M      | Unblocks CI graph  |
| 2   | Remove `@gtcx/workproof` devDep from verification if test moved      | Remediation | `packages/verification/package.json`                                                                                       | S      | Closes cycle       |
| 3   | README: split library 9.x vs DTF Tier 5                              | Remediation | `README.md`                                                                                                                | S      | Spec fidelity      |
| 4   | Reconcile package count in `docs/specs/packages/README.md`           | Remediation | docs                                                                                                                       | S      | Onboarding truth   |
...
| #   | Task                                                          | Layer      | Files                                                                                  | Effort | Why It Matters        |
| --- | ------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------- | ------ | --------------------- |
| 1   | Generate `groth16-zw-diamond-origin.kat.json`                 | Innovation | `artifacts/kat/`, `packages/zkp-kat-vectors/`, `rust/gtcx-zkp/src/bin/generate-kat.rs` | M      | D6 evidence           |
| 2   | Add to `kat-cross-impl-verify.rs` `--all` list                | Evolution  | `rust/gtcx-zkp/src/bin/kat-cross-impl-verify.rs:28-34`                                 | S      | Portable proofs       |
| 3   | Mark DTF-5.2.3 done in workplan                               | Evolution  | `docs/operations/tier-5-workplan-2026-06.md`                                           | S      | P22 truth             |
...
| #   | Task                                               | Layer      | Files                                                        | Effort | Why It Matters      |
| --- | -------------------------------------------------- | ---------- | ------------------------------------------------------------ | ------ | ------------------- |
| 1   | `gh-cocoa-origin` profile config + negative tests  | Innovation | `rust/gtcx-zkp/src/circuit_profiles/`, `packages/workproof/` | M      | Tier-5 breadth      |
| 2   | TS `proveGhCocoaOrigin` + verification bundle test | Evolution  | `packages/crypto/`, `packages/verification/tests/`           | M      | End-to-end          |
| 3   | Five-jurisdiction redacted fixtures                | Innovation | `packages/workproof/tests/` or `docs/audit/evidence/`        | M      | Regulator narrative |
| 4   | UAT protocol template (5.3.3 ops-docs)             | Evolution  | `docs/operations/`                                           | S      | Commercial path     |
...
| 2   | Outbound ticket to gtcx-infrastructure for hub ack           | Remediation | `docs/operations/coordination/to-gtcx-infrastructure-er-1-08-*.md` | S      | OI-X02         |
| 3   | Update `remaining-cross-repo-work-2026-06-02.md`             | Evolution   | `docs/operations/coordination/`                                    | S      | Protocol 24    |
...
| #   | Task                                             | Layer      | Files                                                          | Effort | Why It Matters |
| --- | ------------------------------------------------ | ---------- | -------------------------------------------------------------- | ------ | -------------- |
| 1   | `CircuitRegistry` with semver + deprecation      | Innovation | new `packages/crypto/src/circuit-registry.ts` or rust registry | L      | Tier-5.4       |
| 2   | Load test 1000 proofs/min script + evidence JSON | Innovation | `tools/` or `benchmarks/`                                      | M      | Trust portal   |
| 3   | Trust portal circuit ID column doc (5.4.3)       | Evolution  | `docs/governance/trust-portal.md`                              | S      | Verifier UX    |

### Definition of Done

- Registry rejects unknown profile IDs with structured error.
- Evidence JSON committed under `docs/audit/evidence/`.
...
| #   | Task                                                        | Layer       | Files                              | Effort | Why It Matters    |
| --- | ----------------------------------------------------------- | ----------- | ---------------------------------- | ------ | ----------------- |
| 1   | Evidence index entry for Tier-5 technical exit (5.5.5)      | Evolution   | `docs/audit/evidence/`             | S      | Bank-grade track  |
| 2   | Package fuzz + KAT + threat matrix into vendor zip manifest | Remediation | `docs/security/`, `artifacts/kat/` | M      | EXT-INF-002 input |
| 3   | Zimbabwe sandbox email + LOI tracker (human gate)           | Remediation | `docs/gtm/`                        | S      | GTM               |
| 4   | Track CORE-004 ceremony unblock for D3 M3.2                 | Evolution   | `.baseline/memory/dependencies.md` | S      | Moat D3           |

### Definition of Done

- `docs/audit/evidence/` index lists all KAT + fuzz + CI confirmation paths.
