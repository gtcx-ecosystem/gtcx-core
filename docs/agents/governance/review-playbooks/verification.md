...
**Version:** 1.0.0
**Applies to:** `packages/verification/`, `packages/workproof/`
...
**Rule:** Breaking changes to Zod schemas in `packages/verification/src/types/schemas.ts` and `packages/workproof/src/` (renamed fields, narrowed types, required → required-different, removed fields) require:
...
**Severity:** `high`
**References:** `docs/decisions/013-api-baseline-and-performance-budget-gates.md`, `docs/decisions/002-zod-over-json-schema.md`

---

## 2. Hash-chain integrity preserved in proof bundles

**Rule:** Proof bundles in `packages/verification/src/proofs/` rely on hash-chain linkage. Every bundle entry's `prev_hash` must point to the previous entry. Modifications must not introduce a code path that produces a bundle without computing or signing over `prev_hash`.
...
**Severity:** `critical`
**References:** `docs/decisions/006-hash-chain-audit-trail.md`

---

## 3. QR proof bundle recency window

**Rule:** `packages/verification/src/traced/qr.ts` enforces a recency window on QR-encoded proof bundles. Changes to the window value require:
...
- No corresponding test vector change in `packages/verification/tests/`
- No `docs/security/threat-model.md` update
...
**Rule:** `packages/verification/src/certificates/` accepts a `RevocationChecker` interface (post-Sprint 2) that callers must supply. Removing the interface, defaulting to a no-op, or short-circuiting the check is a critical finding.
...
**Rule:** Modifications to `packages/verification/src/commodities.ts` must preserve the round-trip property of `migrateGoldLotData` and `migrateLegacyLotData` for all existing fixtures in `packages/verification/tests/`.
...
**Rule:** New exports from `packages/verification/src/index.ts` and `packages/workproof/src/index.ts` require an updated `quality/api-surface-baseline.json`. The CI gate `api:check` will fail otherwise; flag preemptively.
...
**Rule:** New proof types added to `packages/verification/src/proofs/` or `packages/workproof/src/predicates/` must include:
...
- No corresponding test fixture in `packages/verification/tests/` or `packages/workproof/tests/`
...
- Diff adds or removes a predicate file in `packages/workproof/src/predicates/definitions/`
...
**Rule:** `packages/workproof/src/` AI validation types must preserve the attestation chain — the signing entity, the validator entity, and the timestamp must all be present and signed over.
