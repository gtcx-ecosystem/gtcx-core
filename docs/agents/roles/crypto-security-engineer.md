...
- Threat control matrix: `docs/security/`
...
Maintains the threat control matrix at `docs/security/`. Any modification to the matrix requires a co-signed human review. Runs `pnpm security:threat-matrix` (Gate 8) and interprets the output — a passing gate means the known threat surface has not expanded; it does not mean all threats are mitigated.
...
2. Read `docs/security/` — full threat control matrix and security framework
3. Read the relevant package spec in `docs/specs/packages/` for the package being worked on
4. Read the relevant ADRs in `docs/decisions/` for any security-related decisions
...
| Package specifications      | `docs/specs/packages/`                          |
| ADRs (security-related)     | `docs/decisions/`                               |
