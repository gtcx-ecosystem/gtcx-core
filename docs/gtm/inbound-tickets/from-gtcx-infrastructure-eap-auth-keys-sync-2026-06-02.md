...
- **File:** `packages/eap/src/sync-intelligence.ts`
- **What it does:**
...
- **File:** `packages/eap/src/cli-sync-bundle.ts`
- **Script:** `pnpm eap:sync-bundle` (from `packages/eap/`)
- **Env vars:**
...
- `packages/eap/src/sync-intelligence.ts` — `rebuildIntelligenceBundleFromEapSecrets()`
- `packages/eap/src/cli-sync-bundle.ts` — CLI entry point
- `packages/eap/package.json` — `eap:sync-bundle` script
