...
`docs/` passes Protocol 1 v2.0 and protects every audit-trail requirement, but its top-level structure is wider than industry best practice and has overlapping concerns. The substance is there; the shape is not exemplary.
...
| **Same-name two-meaning conflict** | `docs/quality/` (human-facing quality docs) vs root `quality/` (CI-generated artifacts) — same word, two scopes, confusing     | Medium   |
...
| **Downstream blast**       | Absolute GitHub URLs from this repo are referenced in: `gtcx-mobile` README, `gtcx-protocols` integration guides, trust portal hosting (`docs/operations/trust-portal-hosting.md`). Each link cascade requires coordination + downstream PRs. |
...
2. **Phase 2 — Fix internal links** (~1.5 hours)
   - `rg -l "docs/decisions"` → rewrite to `docs/architecture/decisions`
   - `rg -l "docs/stack"` → rewrite to `docs/architecture`
   - `rg -l "docs/operations"` → rewrite to `docs/devops`
   - `rg -l "docs/remediation"` → rewrite to `docs/audit`
   - `rg -l "docs/guides"` → rewrite to `docs/reference`
   - Run `pnpm docs:links:check` to confirm zero broken links

3. **Phase 3 — Update README indices** (~30 min)
   - `docs/README.md` top-level table
   - `docs/overview/README.md` if it enumerates dirs
...
4. **Phase 4 — Downstream coordination** (~2 hours, async)
   ...
   - Trust portal absolute URLs: regenerate the index in `docs/operations/trust-portal-hosting.md`
...
5. **Phase 5 — Audit-trail anchor** (~30 min)
   ...
   - Update `docs/audit/internal-completion-audit-YYYY-MM-DD.md` to reflect new 9.5/10 score
...
- Adding new docs: separate initiative (`docs/audit/internal-completion-audit-2026-05-21.md` covers content completeness)
- `quality/` (root) vs `docs/quality/` naming clash: tracked separately; the root `quality/` directory is generated CI output and renaming it has its own blast radius
