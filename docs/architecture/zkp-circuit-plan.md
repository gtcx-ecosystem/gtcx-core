...
2. **Phase B:** Arkworks circuits in `rust/gtcx-zkp` for the above proof types. The first circuits landed are Groth16-based GCI threshold, asset ownership, and location region proofs; Bulletproofs range proofs remain the target for the final `gci_threshold` design.
