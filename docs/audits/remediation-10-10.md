...
- `packages/ai/src/index.ts` grew past the 500-LOC ceiling (551) when SpanEmitter contract + sanitizer-override telemetry were added
...
| ---- | ------------------------------------------------------------------------------------------------ | ---------- | -------------------- | --------------------------------------- | ---------------------------------------- |
| F-1  | `packages/ai/src/index.ts` exceeds 500 LOC (551) — `architecture:check` FAILS                    | **High**   | Phase 1 (cycle 6)    | Repo maintainer (technical)             | None                                     |
| F-2  | `_delete/` files break `docs:check-links` (`uat-evidence-log-DUPLICATE.md` internal links)       | **Medium** | Phase 1, 4 (cycle 6) | Repo maintainer (technical)             | F-3 (final state of `_delete/`)          |
| F-3  | `_delete/` mixed state — restored as tracked but originally untracked staging                    | **Medium** | Phase 4 (cycle 6)    | User decision                           | None                                     |
| F-4  | 6 unpushed commits on `main`                                                                     | Low        | Phase 4 (cycle 6)    | User push                               | None                                     |
| F-5  | `runtime` package aggregator undocumented in `tools/check-package-boundaries.mjs`                | Low        | Phase 1 (carryover)  | Repo maintainer (technical)             | None                                     |
| F-6  | `tracing.ts` dynamic-require branch not covered (1% test coverage gap)                           | Low        | Phase 4 (carryover)  | Repo maintainer (technical)             | None                                     |
| F-7  | `Pkcs11KeyStore` v1 keeps `KeyState` in process memory (no persistence)                          | Low        | Phase 2 (cycle 6)    | Repo maintainer (technical)             | Sprint 5+                                |
| F-8  | Cloud KMS adapter not implemented — sibling to PKCS#11 backend                                   | Low        | Phase 2 (carryover)  | Repo maintainer (technical)             | Sprint 5+                                |
| F-9  | `pnpm pack` workspace-dep reordering breaks reproducibility for packages with `workspace:*` deps | Medium     | Phase 2 (cycle 6)    | Upstream pnpm + workaround              | Upstream OR canonicalization step        |
| F-10 | CI infrastructure failure — sub-3s job startup since 2026-05-09                                  | **Medium** | Phase 3, 4           | User (org-level GitHub Actions billing) | External                                 |
...
### A.1 Decompose `packages/ai/src/index.ts` — closes F-1

**Effort:** 2 hours
**Files:**

- `packages/ai/src/traced.ts` (new) — `traced()` + `withTrace()` (lines 344-547 of current index.ts)
- `packages/ai/src/span-emitter.ts` (new) — `SpanEmitter` interface, `setDefaultSpanEmitter`, `getDefaultSpanEmitter`, internal slot (lines 121-152, 199-235)
- `packages/ai/src/redaction.ts` (new) — `redactSecrets`, `REDACTED_KEY_PATTERNS` (lines 277-305)
- `packages/ai/src/category-logger.ts` (new) — `CategoryLogger`, `createCategoryLogger`, `writeLog`, `safeEmit` (lines 149-242)
- `packages/ai/src/trace-context.ts` (new) — `TraceContext`, `runWithTraceContext`, `getCurrentTraceContext`, async-local-storage helpers (lines 18-66)
- `packages/ai/src/index.ts` — barrel re-export of all above (≤50 LOC)
...
**Effort:** 15 minutes
**Files:** `tools/check-markdown-links.mjs`
...
**Effort:** 10 minutes
**Files:** `tools/check-package-boundaries.mjs`
...
**Effort:** 1 hour
**Files:** `packages/verification/src/tracing.ts` (or wherever the 1%-uncovered branch lives — confirm via coverage report)
...
- `rust/gtcx-crypto/src/pkcs11_keystore/state.rs` (new) — `KeyStateStore` trait + filesystem-backed implementation
- `rust/gtcx-crypto/src/pkcs11_keystore.rs` — accept a `Box<dyn KeyStateStore>` in `Pkcs11KeyStore::new` (with default = in-memory fallback for backwards compat)
- `docs/security/pkcs11-keystore.md` — update Hardening Pass #2 from "documented" to "shipped" and add the API surface
...
**Effort:** 1.5 hours
**Files:** `tools/check-reproducible-build.mjs`
...
**Effort:** 5 minutes
**Action:** open `docs/gtm/09-pre-submission-email-zimbabwe.md`, copy to email client, send to the Zimbabwe sandbox regulator listed in the doc.

**Verification:** sent email captured in `docs/gtm/responses/zimbabwe-2026-05-XX.md` (timestamped); regulator response tracked when it arrives.
...
**Effort:** 30 minutes
**Action:** send Namibia, Zambia, Ghana per `docs/gtm/{10,11,13}-*`. Initiate DRC engagement per `docs/gtm/12-engagement-brief-drc.md`.

### C.2 Track responses

**Effort:** ongoing
**Files:** `docs/gtm/responses/<market>-<date>.md` per response

### C.3 First reference customer case study — closes F-19, F-21

**Effort:** 1 day after first regulator engagement
**Files:** `docs/gtm/14-first-deal-case-study.md`

### C.4 Customer success motion — closes F-18

**Effort:** undefined; emerges post-first-customer
**Output:** SLA, support tier, pricing — currently absent from docs/gtm/, gated on first commercial conversation
...
1. Close the 7 documented gaps in `docs/compliance/soc2-readiness.md` (~6 weeks parallelizable with CPA engagement)
...
**Sequence:** wait until 30+ real PRs have run through the AI CODEOWNER action (so the template has track record). Then extract `.github/scripts/codeowner-review/` + `docs/agents/governance/` to a new public repo `gtcx-ecosystem/ai-native-governance-template`. Distribute via HN / X / conference talk.
...
1. A.1 — decompose `ai/src/index.ts` (2h)
...
**Phase C DoD:** ≥1 sandbox regulator response captured in `docs/gtm/responses/`; first case study skeleton drafted.
