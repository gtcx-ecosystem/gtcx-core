...
- Threat control matrix: `_sop/2-docs/3-engineering/7-security/`
...
Maintains the threat control matrix at `_sop/2-docs/3-engineering/7-security/`. Any modification to the matrix requires a co-signed human review. Runs `pnpm security:threat-matrix` (Gate 8) and interprets the output — a passing gate means the known threat surface has not expanded; it does not mean all threats are mitigated.
...
2. Read `_sop/2-docs/3-engineering/7-security/` — full threat control matrix and security framework
3. Read the relevant package spec in `_sop/2-docs/5-specs/4-backend/packages/` for the package being worked on
4. Read the relevant ADRs in `_sop/2-docs/3-engineering/6-decisions/` for any security-related decisions
...
| Package specifications      | `_sop/2-docs/5-specs/4-backend/packages/`       |
| ADRs (security-related)     | `_sop/2-docs/3-engineering/6-decisions/`        |
