...
Currently, safety rules are documented constraints enforced by convention and human review. If `1-agentic` provides machine-enforced safety policies, the rules in `_sop/1-agents/4-workflows/safety-rules.md` will need to be expressed in whatever policy format `1-agentic` supports.
