...
> Canonical structure for `/docs` across all [Organization Name] repositories.

---

## SOP Repo Structure (This Folder)

```
repo/2-docs/
├── 1-product/      # Product strategy, opportunity, market, economics, roadmap
├── 2-company/      # Brand, sales/marketing, partnerships, fundraising, legal, people
├── 3-engineering/  # Principles, system design, tech stack, deployment, compliance, ADRs
├── 4-devops/       # CI/CD, environments, runbooks, QA, analytics, monitoring, release
├── 5-specs/        # Product, design, frontend, backend, data, testing specs
└── 6-cannon/       # Glossary, research, GitBook content, changelog, archived
```

---

This standard defines how documentation is organized in every repo. The structure is designed from first principles: 6 core folders, each answering a distinct question.

---

## Structure (All Other Repos)

All other repos use the **unnumbered** standard root folders:
...
### `01-architecture/` — How the system works

System design, data models, design principles, and architectural decisions.

| Subfolder          | Purpose                                                                   | Create when...                       |
| ------------------ | ------------------------------------------------------------------------- | ------------------------------------ |
| `06-decisions/`    | Architecture Decision Records (ADRs)                                      | First significant technical decision |
| `03-data-models/`  | Database schemas, content models, data dictionaries                       | First data model is designed         |
| `07-deployment/`   | Deployment architecture, cloud topology, environments                     | Infrastructure is provisioned        |
| `08-monitoring/`   | Observability, alerting, SLOs, dashboards                                 | First production deployment          |
| `01-principles/`   | Design principles that shape how products are built                       | Core principles are established      |
| `09-global-south/` | Offline-first, low-bandwidth, multi-language, SMS/USSD design constraints | Building for frontier markets        |

**README.md** in each folder is the primary navigation index. Standards and authoring rules live in `AGENTIC-GUIDE.md` within that section.
...
**What does NOT belong here**: API endpoint docs for external users (→ `06-gitbook/api/`), per-service specs (→ `02-specs/`).

---

### `02-specs/` — What we're building
...
| `packages/`       | It is a library or SDK repo exporting importable packages — one subfolder per package or crate |
...
**This is the ONE folder whose contents differ per repo.** Everything project-specific lives here — product vision, business strategy, brand guidelines, service specs, sprint plans. The other 5 core root folders are universal.
...
**What does NOT belong here**: System architecture (→ `01-architecture/`), how-to guides for developers (→ `03-engineering/`).
...
**What does NOT belong here**: Deployment runbooks (→ `04-operations/`), API docs for users (→ `06-gitbook/`), design principles (→ `01-architecture/`).

---

### `04-operations/` — How to run it
...
**What does NOT belong here**: CI/CD pipeline config (→ `03-engineering/`), system architecture (→ `01-architecture/`).

---

### `05-reference/` — Where to look things up
...
**What does NOT belong here**: Active architecture docs (→ `01-architecture/`), product specs (→ `02-specs/`).

---

### `06-gitbook/` — What users see
...
**What does NOT belong here**: Internal architecture (→ `01-architecture/`), internal specs (→ `02-specs/`), developer contributing guides (→ `03-engineering/`).

---

## Principles

### 1. Six core folders, one question each
...
| Question                      | Folder             |
| ----------------------------- | ------------------ |
| How does the system work?     | `01-architecture/` |
| What are we building?         | `02-specs/`        |
| How do I build/contribute?    | `03-engineering/`  |
| How do I run/maintain this?   | `04-operations/`   |
| Where do I look something up? | `05-reference/`    |
| What do external users need?  | `06-gitbook/`      |
...
### 4. Project-specific content lives in `02-specs/`

The other 5 core folders have universal subfolders that work across any repo. Only `02-specs/` contains project-specific names (service names, product names, business units). This is by design.
...
**Library / SDK repo** (add `packages/` under `specs/`, omit `gitbook/` if no external docs):

```bash
mkdir -p docs/{architecture/{decisions,data-models,deployment,monitoring,principles},specs/{_project/{overview,planning},packages},engineering/{guides,security,testing,devops},operations/{runbooks,compliance},reference/{archived,glossary,changelog}}
```

Then:

1. Copy this README into `docs/README.md`
2. Add a README to each root folder explaining what belongs there
3. Add `.gitkeep` to empty subfolders so git tracks them

---

_Standard version: 1.2 — March 2026_
