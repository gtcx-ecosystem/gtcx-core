...
| SC.L2-3.13.11 | 3.13.11      | FIPS-validated crypto (pathway defined)              | `_sop/2-docs/3-engineering/7-security/fips-assessment.md`             |
...
| ------------------------------------- | -------- | ------------------------------------------------------ | ------ |
| FIPS mode flag not yet implemented    | High     | Add `GTCX_FIPS_MODE` env var to switch crypto defaults | M      |
| No DISA STIG compliance documentation | High     | Map STIGs to library controls                          | L      |
| No code signing on Git commits        | Medium   | Enable GPG signing in branch protection                | XS     |
| ZKP verification is placeholder       | Medium   | Wire Rust arkworks via NAPI or document as non-CUI     | L      |
| No ITAR assessment                    | Low      | Assess dual-use classification of crypto exports       | S      |
...
- `_sop/2-docs/3-engineering/7-security/fips-assessment.md`
- `_sop/2-docs/3-engineering/7-security/nist-800-53-mapping.md`
