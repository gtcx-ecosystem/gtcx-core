 ...
 *   node scripts/check-cross-repo-dependencies.mjs --strict
 */
...
function bridgeUpdatedWithinDays(days = 14) {
  const bridge = read('docs/operations/coordination/cross-repo-agent-bridge.md');
...
const remaining = read('docs/operations/coordination/remaining-cross-repo-work-2026-06-02.md');
...
try {
  execSync(`node scripts/check-coordination-hygiene.mjs${strict ? ' --strict' : ''}`, {
    cwd: ROOT,
    stdio: 'pipe',
    encoding: 'utf8',
  });
...
console.error('\nFix: docs/operations/agent-git-workflow.md § Cross-repo dependency checks');
