 ...
 *
 * Usage:
 *   node scripts/check-coordination-hygiene.mjs
 *   node scripts/check-coordination-hygiene.mjs --strict   # exit 1 on warnings
 */
...
function hasInboundOrCoordinationDoc() {
  const dirs = [
    ...
    'docs/coordination',
    'docs/operations/inbound-tickets',
  ];
...
function pointerListsBlockers() {
  const pointer = read('docs/audit/agent-work-pointer.md');
...
function roadmapHasP0Blocked() {
  const roadmap = read('docs/audit/execution-roadmap.md');
...
if (p0Blocked && !hasDeps && !hasInbound && !pointerListsBlockers()) {
  warnings.push(
    '  → Or add docs/gtm/inbound-tickets/from-<repo>-<topic>-YYYY-MM-DD.md / docs/coordination/ brief.',
  );
  warnings.push(
